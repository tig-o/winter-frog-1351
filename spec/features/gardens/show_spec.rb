...
RSpec.describe 'gardens show page', type: :feature do
    ...
    xit 'EXTENSION: list of plants is sorted by the number of plants that appear in any of that gardens plots from most to least' do
        sun_garden = Garden.create!(name: "Sun Garden", organic: true)
        turing_garden = Garden.create!(name: "Turing Garden", organic: true)
        ...
        PlotPlant.create!(plot: plot3, plant: pineapple)
        PlotPlant.create!(plot: plot1, plant: watermelon)
        ...
        visit "/gardens/#{sun_garden.id}"

        within "#list-of-plants" do
            expect("Pineapple").to appear_before("Watermelon") #need to install orderly gem but time is up, now pushing
            expect("Watermelon").to appear_before("Kiwi")
            expect(page).to have_content("Kiwi")
